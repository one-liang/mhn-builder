...
系統 SHALL 提供配裝模擬器頁面（路由 `/build`），頁面包含 7 個裝備槽位（武器、頭部、胸部、腕部、腰部、腿部、漂流石）。每個槽位 SHALL 顯示目前已選裝備名稱，若未選擇則顯示「未選擇」提示文字。

#### Scenario: View build simulator
- **WHEN** 使用者導航至 `/build`
- **THEN** 頁面 SHALL 顯示 7 個裝備槽位格子，包含武器、頭/胸/腕/腰/腿防具、漂流石
...
- **WHEN** 使用者已選擇某裝備至槽位
- **THEN** 該槽位 SHALL 顯示裝備名稱、稀有度顏色標示，以及最多 2 個技能標籤
...
#### Scenario: Open driftstone selector
- **WHEN** 使用者點擊漂流石槽位
- **THEN** 系統 SHALL 開啟顯示 15 種神秘漂流石的選擇器 Modal
...
系統 SHALL 在配裝模擬器頁面下方顯示技能統計面板，即時聚合所有已選裝備（含漂流石可能技能）的技能等級加總。每個技能 SHALL 顯示技能名稱、當前等級（進度條）、最高等級。
...
系統 SHALL 在配裝頁面提供「分享配裝」按鈕。點擊後將當前配裝序列化為 URL query string（格式：`/build?w=<slug>&h=<slug>&c=<slug>&a=<slug>&wa=<slug>&l=<slug>&d=<type>`），並複製至剪貼板。
